...
test.serial('Verify GitHub auth', async (t) => {
  ...
  await t.notThrowsAsync(t.context.m.verifyConditions({}, { cwd, env: environment, logger: t.context.logger, options }))
...
test.serial('Verify GitHub auth and assets config', async (t) => {
  ...
  await t.notThrowsAsync(
    t.context.m.verifyConditions({ assets }, { cwd, env: environment, logger: t.context.logger, options })
  )
...
test.serial('Throw SemanticReleaseError if invalid config', async (t) => {
  ...
  const options = {
    publish: [
      ...
      { assets, assignees, failComment, failTitle, labels, path: '@semantic-release/github', successComment }
    ],
  ...
  const errors = [
    ...(await t.throwsAsync(
      t.context.m.verifyConditions({}, { cwd, env: environment, logger: t.context.logger, options })
    ))
  ]
...
test.serial('Publish a release with an array of assets', async (t) => {
  ...
  const environment = { GITHUB_TOKEN: 'github_token' }
  const assets = [
    { name: 'upload_file_name.txt', path: ['upload.txt'] },
    { label: 'Other File', name: 'other_file.txt', path: ['upload_other.txt'] }
  ]
  ...
  const githubUpload1 = upload(environment, {
  ...
  const githubUpload2 = upload(environment, {
  ...
  const result = await t.context.m.publish(
    { assets },
    { branch: { main: true, type: 'release' }, cwd, env: environment, logger: t.context.logger, nextRelease, options }
  )
...
test.serial('Publish a release with release information in assets', async (t) => {
  ...
  const result = await t.context.m.publish(
    { assets },
    { branch: { type: 'release' }, cwd, env: environment, logger: t.context.logger, nextRelease, options }
  )
...
test.serial('Comment and add labels on PR included in the releases', async (t) => {
  ...
  await t.context.m.success(
    { failTitle },
    { commits, cwd, env: environment, logger: t.context.logger, nextRelease, options, releases }
  )
...
test.serial('Open a new issue with the list of errors', async (t) => {
  ...
  await t.context.m.fail(
    { failTitle },
    { branch: { name: 'master' }, cwd, env: environment, errors, logger: t.context.logger, options }
  )
...
test.serial('Verify, release and notify success', async (t) => {
  ...
  const repo = 'test_repo'
  const environment = { GITHUB_TOKEN: 'github_token' }
  const assets = ['upload.txt', { label: 'Other File', name: 'other_file.txt', path: 'upload_other.txt' }]
  const failTitle = 'The automated release is failing 🚨'
  const options = {
    publish: [{ path: '@semantic-release/npm' }, { assets, path: '@semantic-release/github' }],
