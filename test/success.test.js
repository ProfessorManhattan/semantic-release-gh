...
test.serial(
  ...
  async (t) => {
    ...
    const prs = [
      ...
      { body: 'Fixes #3', number: 2, pull_request: {}, state: 'closed' }
    ]
    ...
    await success(pluginConfig, { commits, env: environment, logger: t.context.logger, nextRelease, options, releases })
...
test.serial(
  ...
  async (t) => {
    const owner = 'test_user'
    const repo = 'test_repo'
    const environment = { GH_PREFIX: 'prefix', GH_TOKEN: 'github_token', GH_URL: 'https://custom-url.com' }
    const failTitle = 'The automated release is failing 🚨'
    const pluginConfig = { failTitle }
    const prs = [
      { number: 1, pull_request: {}, state: 'closed' },
    ...
    const nextRelease = { channel: 'next', version: '1.0.0' }
...
test.serial('Ignore missing and forbidden issues/PRs', async (t) => {
  ...
  const prs = [
    { number: 1, pull_request: {}, state: 'closed' },
    { body: 'Fixes #4', number: 2, pull_request: {}, state: 'closed' },
    { body: 'Fixes #5', number: 3, pull_request: {}, state: 'closed' }
  ]
  const options = { branch: 'master', repositoryUrl: `https://github.com/${owner}/${repo}.git` }
  const commits = [
    { hash: '123', message: 'Commit 1 message\n\n Fix #1' },
...
test.serial('Add custom comment and labels', async (t) => {
  ...
  const commits = [{ hash: '123', message: 'Commit 1 message' }]
  const nextRelease = { channel: 'next', version: '2.0.0' }
  const releases = [{ name: 'GitHub release', url: 'https://github.com/release' }]
  const github = authenticate(environment)
    .get(`/repos/${owner}/${repo}`)
    .reply(200, { full_name: `${owner}/${repo}` })
...
test.serial('Add custom label', async (t) => {
  ...
  const environment = { GITHUB_TOKEN: 'github_token' }
  const failTitle = 'The automated release is failing 🚨'
  const pluginConfig = { failTitle, releasedLabels: ['custom label'] }
...
test.serial('Comment on issue/PR without ading a label', async (t) => {
  ...
  const environment = { GITHUB_TOKEN: 'github_token' }
  const failTitle = 'The automated release is failing 🚨'
  const pluginConfig = { failTitle, releasedLabels: false }
  const prs = [{ number: 1, pull_request: {}, state: 'closed' }]
  const options = { repositoryUrl: `https://github.com/${owner}/${repo}.git` }
  const lastRelease = { version: '1.0.0' }
  const commits = [{ hash: '123', message: 'Commit 1 message' }]
...
test.serial('Editing the release to include all release links at the bottom', async (t) => {
  ...
  const environment = { GITHUB_TOKEN: 'github_token' }
  const failTitle = 'The automated release is failing 🚨'
  const pluginConfig = { addReleases: 'bottom', releasedLabels: false }
  const prs = [{ number: 1, pull_request: {}, state: 'closed' }]
  const options = { repositoryUrl: `https://github.com/${owner}/${repo}.git` }
  const nextRelease = { gitTag: 'v1.0.0', name: 'v1.0.0', notes: 'Test release note body', version: '2.0.0' }
  const lastRelease = { version: '1.0.0' }
  const commits = [{ hash: '123', message: 'Commit 1 message' }]
  const releaseUrl = `https://github.com/${owner}/${repo}/releases/${nextRelease.version}`
  const releaseId = 1
  const releases = [
    { id: releaseId, name: 'GitHub release', url: 'https://github.com/release' },
...
test.serial('Editing the release to include all release links at the top', async (t) => {
  ...
  const environment = { GITHUB_TOKEN: 'github_token' }
  const failTitle = 'The automated release is failing 🚨'
  const pluginConfig = { addReleases: 'top', releasedLabels: false }
  const prs = [{ number: 1, pull_request: {}, state: 'closed' }]
  const options = { repositoryUrl: `https://github.com/${owner}/${repo}.git` }
  const nextRelease = { gitTag: 'v1.0.0', name: 'v1.0.0', notes: 'Test release note body', version: '2.0.0' }
  const lastRelease = { version: '1.0.0' }
...
test.serial('Editing the release to include all release links with no additional releases (top)', async (t) => {
  ...
  const commits = [{ hash: '123', message: 'Commit 1 message' }]
  const releaseId = 1
  const releases = [{ id: releaseId, name: 'GitHub release', url: 'https://github.com/release' }]
  const github = authenticate(environment)
    .get(`/repos/${owner}/${repo}`)
    .reply(200, { full_name: `${owner}/${repo}` })
    .get(
